...
library(xtable)
library(kinship2)
presamplesheet <- read.csv("SampleSheet.csv", header=TRUE, row.names=NULL)
```
...
if (file.exists("report_data/pedigree.ped")) {
  pedigree_data <- read.table("report_data/pedigree.ped", header=F, col.names=c("ped","id","father","mother","sex","affected"), stringsAsFactors=FALSE, na.strings = "0")
  pedigree <- pedigree(
   id=pedigree_data$id,
...
bamstats_data <- read.delim("report_data/all_samples.bamstats", sep="\t", header=TRUE, row.names="SAMPLE_NAME", check.names=FALSE)
tot.aligned <- sum(bamstats_data[,2])
```
```{r hsmetricsLoad, results='hide', echo=FALSE}
hsmetrics_data <- read.delim("report_data/all_samples.hsmetrics", sep="\t", header=TRUE, row.names="SAMPLE_NAME", check.names=FALSE)
qc_data_summary <- cbind(formatC(bamstats_data[,1], format="e"),formatC(bamstats_data[,2], format="e"), round(bamstats_data[,3], digits=2), round(hsmetrics_data$MEAN_TARGET_COVERAGE, digits=2))
...
if (file.exists("")) {
  healty_exomes <- read.table("", stringsAsFactors=F)$V1
} else {
  healty_exomes <- NA
}
```
...
tiers_count <- read.table("report_data/variants_count.txt", row.names=1, col.names=c("","Variants"))
...
vcf.coverage.data <- read.table("report_data/all_samples.vcfcoverage", header=T, stringsAsFactors=FALSE)
