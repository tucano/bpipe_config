 ...
 */
 Bpipe.run {
    set_stripe_gfu.using(test:true) + "_R*_%.fastq.gz" * [split_fastq_gfu.using(SPLIT_READS_SIZE: 2000000, paired: true, test : true)] +
    "%.fastq" * [soapsplice_prepare_headers_gfu.using(test:true)] +
    "read*_%.fastq" * [align_soapsplice_gfu.using(paired: true, test: true, compressed: false) ] + verify_bam_gfu.using(test:true) +
    merge_bam_gfu.using(rename:true, test:true) + merge_junc_gfu.using(test:true) + mark_duplicates_gfu.using(test:true) + bam_flagstat_gfu.using(test:true)
 }
